...
$thumbRoot = $_SERVER['DOCUMENT_ROOT'] . '/engine/tmp/' . $_SERVER['HTTP_HOST'];
$thumbImagePath =  $thumbRoot . '/thumbs/' . str_replace('/', '-', $_GET["file"]);

if (is_file($thumbImagePath)) {
  print file_get_contents($thumbImagePath);
  die();
}


mkdir($_SERVER['DOCUMENT_ROOT'] . '/engine/tmp/' . $_SERVER['HTTP_HOST']);
mkdir($_SERVER['DOCUMENT_ROOT'] . '/engine/tmp/' . $_SERVER['HTTP_HOST'] . '/thumbs');


$sImagePath = $thumbRoot . '/wiki/' . $_GET["file"];
