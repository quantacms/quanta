...
// --- Scenario 6: external overwrite observed (verify_reads=always). ---------
...
eq(QuantaDb::get('ext1'), ['v' => 'external-longer-value'], 'external overwrite observed');
...
// External move: index heals to the new location.
seed_node($root, 'home/sub', []);
rename("$root/home/ext2", "$root/home/sub/ext2");
ok(str_ends_with((string) QuantaDb::path('ext2'), '/home/sub/ext2'), 'moved node re-resolved');
eq(QuantaDb::meta('ext2')['father'], 'sub', 'father healed after move');

// External delete: stale index row cleaned up.
exec('rm -rf ' . escapeshellarg("$root/home/ext1"));
eq(QuantaDb::get('ext1'), null, 'externally deleted node -> null');
...
// --- Scenario 10: brand-new index file rebuilt from the files alone. --------
$fresh = $GLOBALS['__qdb_base'] . '/fresh-index.sqlite';
[$code, $out, $err] = wait_worker(spawn_worker('reindex_fresh.php', [], [
    'QUANTA_DB_INDEX_PATH' => $fresh,
]));
eq($code, 0, 'fresh-index worker exited cleanly' . ($code ? " ($err)" : ''));
$res = json_decode($out, true);
eq($res['reindex']['nodes'], 5, 'fresh index: all nodes rebuilt');
eq($res['reindex']['links'], 1, 'fresh index: links rebuilt');
eq($res['children_sub'], ['ext2', 'ext3'], 'fresh index: children query');
eq($res['links_ext3'], ['cont'], 'fresh index: links query');
eq($res['stats_nodes'], 5, 'fresh index: stats.nodes');
eq($res['get_ext3'], ['v' => 3], 'fresh index: documents readable');
...
eq($s['contract'], '1.0', 'stats.contract');
...
eq(QuantaDb::version(), 'ext/1.0', 'version()');
