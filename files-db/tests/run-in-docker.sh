...
#
# Usage:
#   sh files-db/tests/run-in-docker.sh              # build what is missing, run both
#   sh files-db/tests/run-in-docker.sh conformance  # just the fast one
#   sh files-db/tests/run-in-docker.sh parity
#   QDB_REBUILD=1 sh files-db/tests/run-in-docker.sh   # force a rebuild first
#
...
# suite result:
#
#   mkdir /tmp/base && git archive HEAD files-db | tar -x -C /tmp/base
#
...
run_conformance() {
    ...
    docker run --rm --shm-size="$SHM" -v "$SRC/files-db/tests:/tests:ro" "$BUILDER_IMG" \
...
run_parity() {
    ...
    docker run --rm --shm-size="$SHM" -v "$SRC/files-db/tests:/tests:ro" "$FULL_IMG" \
