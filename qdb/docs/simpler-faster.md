# qdb: simpler to use, and faster

A design review of the read path end to end — PHP caller → extension → shared
memory — with the concrete changes that would make qdb cheaper to call and
...
Sources read: `qdb/src/{lib,image,model,store,shm,config,metrics,php_abi}.rs`,
`qdb/src/bin/qdbd.rs`, `qdb/docs/{api-contract,how-it-works,usage}.md`,
`qdb/docs/quanta_db.stub.php`, and the Quanta/hili call sites
...
qdb did the hard part correctly. `QuantaDb::getObject()` on a warm segment
...
qdb removed, and they are still there, still paying.
...
2. **A syscall-based cache still sits in front of the syscall-free lookup**
   ...
   an `exec(find)` that qdb already eliminated.
...
- **(a) Cache the target, not the link.** Store the resolved path in
  `$node_paths[$folder]` and return it directly. Pure PHP, no qdb
  ...
  for nothing.
- **(b) When `db()->coherent()`, ask qdb first and skip layers 2 and 4
...
becomes the *cheaper* option, not the more expensive one. And qdb's own
...
qdb already has a documented failure mode where it degrades silently and
