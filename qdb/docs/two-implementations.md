...
The safety net already exists and is unusually good for this: `qdb/tests/
...
Implemented. The parity suite (`qdb/tests/quanta/`) runs green in all three
modes — **774 assertions, 0 failures** across `noext`, `fallback` and `daemon` —
and the extension's own conformance suite (`qdb/tests/php/`, 73 assertions)
is unchanged.
