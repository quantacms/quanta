...
/// Release a mapping obtained from `map_file`.
///
/// The extension and the daemon map the arena once and keep it for the life of
/// the process, so neither needs this. A long-running *reader* does: it has to
/// re-open the arena periodically (see `qdbstat --listen`), and without an
/// unmap that would leak a page per refresh.
///
/// # Safety
/// `m` must come from `map_file` and must not be used afterwards.
pub unsafe fn unmap(m: *mut Metrics) {
    libc::munmap(m as *mut libc::c_void, SIZE);
}
