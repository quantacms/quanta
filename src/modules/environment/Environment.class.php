...
class Environment extends DataContainer {
  ...
  private function loadModules($dir, $mod_type) {
    $modules = $this->scanDirectory($dir, array('type' => DIR_MODULES));
 ...
 */
  ...
  public function runModule($module) {
    ...
    $qtags_path = $module['path'] . '/qtags/';
    /**
     * Autoload module's qtags.
     */
    if (is_dir($qtags_path)) {
      $qtags = $this->scanDirectory($qtags_path);
      foreach ($qtags as $qtag) {
        require_once($qtags_path . $qtag);
      }
    }
