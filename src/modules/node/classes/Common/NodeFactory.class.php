...
class NodeFactory
{
  ...
  {
    $node = new Node($env, Node::NODE_NEW, $father, $lang);
    return $node;
  }
  ...
  public static function unlinkNodes($env, $symlink_name, $symlink_folder, $vars = array())
  {
    ...
    // (docs/files-db/api-contract.md §9). On any error, fall through to
    ...
    if (!$symlink_folder_node->exists) {
      new Message(
        $env,
        t(
          'Error: could not unlink !symlink_name from !symlink_folder. !symlink_folder doesn\'t exist',
      ...
      switch ($if_not_exists) {
        case 'error':
          new Message(
            $env,
            t(
              'Error: could not unlink !symlink_name from !symlink_folder. !symlink_folder/!symlink_name doesn\'t exist',
              array('!symlink_name' => $symlink_name, '!symlink_folder' => $symlink_folder)
            ),
            Message::MESSAGE_ERROR
          );
  ...
  public static function linkNodes($env, $source_node, $symlink_folder, $vars = array())
  {
    ...
    // (docs/files-db/api-contract.md §9). Custom symlink names and the
  ...
  public static function updateTemplate($env, $tpl_file, $tpl_contents)
  {
    ...
    if (!is_file($tpl_file)) {
      new Message($env, 'Warning: trying to update a non-existing template: ' . $tpl_file, Message::MESSAGE_ERROR);
    } else {
      // Write the tpl file.
      $f = fopen($tpl_file, 'w');
  ...
  public static function requestAction(Environment $env, $action, array $form_data)
  {
    ...
    $language = isset($form_data['language']->value) ? (array_pop($form_data['language']->value)) : Localization::LANGUAGE_NEUTRAL;
    ...
    // path based on title.
    if (($action == Node::NODE_ACTION_ADD)) {
    ...
    } elseif ($action == Node::NODE_ACTION_DUPLICATE) {
      $node_name = $env->getCandidatePath($form_data['edit-title']);
      $father = $form_data['edit-father'];
    } else {
      $node_name = $form_data['edit-path'];
    ...
    switch ($action) {
      case Node::NODE_ACTION_ADD:
      case Node::NODE_ACTION_EDIT:
      case Node::NODE_ACTION_DUPLICATE:
      case Node::NODE_ACTION_CHANGE_AUTHOR:
        if ($action == Node::NODE_ACTION_ADD || !$node->exists || (!empty($language) && $node->father->exists)) {
        ...
        if ($access_check) {
          ...
          if ($node->validate() && $validation_status && $vars['form_validated']) {
            ...
            if (isset($form_data['shadow_response']) && $form_data['shadow_response']) {
              ...
              $translated_text = Localization::translatableText($env, 'Lo farò più tardi', 'will-do-it-later');
      ...
      Node::NODE_ACTION_DELETE:
        ...
        $has_access = (NodeAccess::check($env, Node::NODE_ACTION_DELETE, array('node' => $node)));
        if ($has_access) {
          // Delete the node...
          $node->delete();
  ...
  public static function validateFormData(Environment $env, $full_form_data)
  {
    ...
    foreach ($full_form_data as $form_item => $form_item_data) {
      ...
      $form_state = null;
      $input_item = FormFactory::createInputItem($env, $attributes, $form_state);
      $input_item->validate();
  ...
  public static function shadowMessage($env, $message, $form_item_name)
  {
    return new Message(
      ...
      $message,
      Message::MESSAGE_WARNING,
      Message::MESSAGE_TYPE_SCREEN,
      Message::MESSAGE_NOMODULE,
      $form_item_name
    );
  }
  ...
  public static function duplicate($env, $source_node, $new_node_name, $father, $language = Localization::LANGUAGE_NEUTRAL, $subnodes = true, $overrides = array(), $overrides_sub_nodes = array(), $exclude = '')
  {
    ...
    if ($subnodes) {
      $source_sub_nodes = new DirList($env, $source_node->name, null, [], 'node');
  ...
  public static function changeAuthor($env, $node, $author, $subnodes = true, $payload = array())
  {
    ...
    if (!$node->hasTranslation($default_lang)) {
      $node->setLanguage(Localization::LANGUAGE_NEUTRAL);
    }
    $node->save();
    if ($subnodes) {
      $subnodes = new DirList($env, $node->name, null, [], 'node');
